...
State SendFrame      = State(send_frame);
...
void setup() {
  ...
  Serial.println(" OpTh Master");
  Serial.println("- MvdBurg 2009 - serial amvv - encoder amvv");
...
void loop() {
  ...
  while (1) {
    OT_StateMachine.transitionTo(SendFrame).update();
